...
public class AnalyticsService {
    ...
    public List<Map<String, Object>> recentInquiries(int limit) {
        // Build lightweight DTOs to avoid serializing full entity graph
        return inquiryRepository.findAll().stream()
                ...
                .map(inq -> {
                    ...
                    m.put("updatedAt", inq.getUpdatedAt());
                    // Minimal nested property info
                    Map<String, Object> p = new HashMap<>();
                    if (inq.getProperty() != null) {
                        p.put("id", inq.getProperty().getId());
                        p.put("title", inq.getProperty().getTitle());
                        p.put("city", inq.getProperty().getCity());
                    }
                    m.put("property", p);
                    return m;
                })
