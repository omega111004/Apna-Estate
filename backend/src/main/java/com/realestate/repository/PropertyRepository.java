...
public interface PropertyRepository extends JpaRepository<Property, Long> {
    ...
    @Query("SELECT p FROM Property p WHERE p.approvalStatus = 'APPROVED'")
    ...
    @Query("SELECT p FROM Property p WHERE p.approvalStatus = 'APPROVED' AND " +
    ...
    // Get distinct cities (all)
    @Query("SELECT DISTINCT p.city FROM Property p WHERE p.city IS NOT NULL ORDER BY p.city")
    List<String> findDistinctCities();

    // Get distinct cities for approved listings only (for public stats)
    @Query("SELECT DISTINCT p.city FROM Property p WHERE p.city IS NOT NULL AND p.approvalStatus = 'APPROVED' ORDER BY p.city")
    ...
    long countByOwner_IdAndStatus(Long ownerId, Property.PropertyStatus status);
