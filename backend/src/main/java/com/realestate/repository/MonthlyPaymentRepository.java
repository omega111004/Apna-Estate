...
public interface MonthlyPaymentRepository extends JpaRepository<MonthlyPayment, Long> {
    ...
    @Query("SELECT mp FROM MonthlyPayment mp WHERE " +
           "mp.rentBooking.tenant.id = :tenantId " +
    ...
    @Query("SELECT mp FROM MonthlyPayment mp WHERE " +
           "mp.rentBooking.owner.id = :ownerId " +
           "AND mp.status = :status")
    ...
    @Query("SELECT COUNT(mp) FROM MonthlyPayment mp WHERE mp.rentBooking.owner.id = :ownerId " +
    ...
    @Query("SELECT COUNT(mp) FROM MonthlyPayment mp WHERE mp.rentBooking.owner.id = :ownerId AND mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.PENDING")
    long countPendingByOwner(@Param("ownerId") Long ownerId);

    @Query("SELECT COUNT(mp) FROM MonthlyPayment mp WHERE mp.rentBooking.owner.id = :ownerId AND mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.OVERDUE")
    long countOverdueByOwner(@Param("ownerId") Long ownerId);

    @Query("SELECT COUNT(mp) FROM MonthlyPayment mp WHERE mp.rentBooking.owner.id = :ownerId AND mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.PAID")
    ...
    @Query("SELECT COALESCE(SUM(mp.amount), 0) FROM MonthlyPayment mp WHERE mp.rentBooking.owner.id = :ownerId AND mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.PENDING")
    java.math.BigDecimal sumPendingAmountByOwner(@Param("ownerId") Long ownerId);

    @Query("SELECT COALESCE(SUM(mp.amount + COALESCE(mp.lateFee, 0)), 0) FROM MonthlyPayment mp WHERE mp.rentBooking.owner.id = :ownerId AND mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.OVERDUE")
    ...
    @Query("SELECT COALESCE(SUM(mp.amount + COALESCE(mp.lateFee, 0)), 0) FROM MonthlyPayment mp WHERE mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.PAID AND mp.paidDate BETWEEN :start AND :end " +
           "AND mp.rentBooking.owner.id = :ownerId")
    ...
    @Query("SELECT COALESCE(SUM(mp.amount + COALESCE(mp.lateFee, 0)), 0) FROM MonthlyPayment mp WHERE mp.status = com.realestate.entity.MonthlyPayment$PaymentStatus.PAID AND mp.paidDate BETWEEN :start AND :end " +
           "AND mp.rentBooking.property.id = :propertyId")
