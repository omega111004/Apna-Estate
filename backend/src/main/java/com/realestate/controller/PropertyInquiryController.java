...
public class PropertyInquiryController {
    ...
    public ResponseEntity<?> sendMessage(@PathVariable Long inquiryId, @Valid @RequestBody SendMessageRequest request) {
        ...
        if (messageType == ChatMessage.MessageType.PRICE_OFFER || messageType == ChatMessage.MessageType.PRICE_COUNTER) {
            if (request.getPriceAmount() != null) {
                inquiry.setOfferedPrice(request.getPriceAmount());
            }
            inquiry.setStatus(PropertyInquiry.InquiryStatus.NEGOTIATING);
        } else if (messageType == ChatMessage.MessageType.PRICE_ACCEPT && request.getPriceAmount() != null) {
            inquiry.setAgreedPrice(request.getPriceAmount());
    ...
    public ResponseEntity<?> updateInquiryStatus(@PathVariable Long inquiryId, @RequestParam String status) {
        ...
        // Automatically sync property status when a deal is completed
        if (newStatus == PropertyInquiry.InquiryStatus.PURCHASED) {
            Property property = inquiry.getProperty();
            if (property != null && property.getStatus() != Property.PropertyStatus.SOLD) {
    ...
    // Admin endpoint to sync property status from inquiry status
    @PostMapping("/{inquiryId}/sync-property-status")
    @PreAuthorize("hasAnyRole('ADMIN','AGENT')")
    public ResponseEntity<?> syncPropertyStatus(@PathVariable Long inquiryId) {
        try {
            Optional<PropertyInquiry> inqOpt = inquiryRepo.findById(inquiryId);
            if (inqOpt.isEmpty()) {
                return ResponseEntity.notFound().build();
            }

            PropertyInquiry inquiry = inqOpt.get();
            Property property = inquiry.getProperty();

            System.out.println("=== SYNCING PROPERTY STATUS ===");
            System.out.println("Inquiry ID: " + inquiry.getId());
            System.out.println("Inquiry Status: " + inquiry.getStatus());
            System.out.println("Property ID: " + property.getId());
            System.out.println("Property Current Status: " + property.getStatus());

            // Sync based on inquiry status
            if (inquiry.getStatus() == PropertyInquiry.InquiryStatus.PURCHASED) {
                property.setStatus(Property.PropertyStatus.SOLD);
                propertyRepo.save(property);
                System.out.println("Property updated to SOLD");
                
                Map<String, Object> response = new HashMap<>();
                response.put("message", "Property status synced to SOLD");
                response.put("propertyId", property.getId());
                response.put("inquiryId", inquiry.getId());
                response.put("propertyStatus", property.getStatus().name());
                response.put("inquiryStatus", inquiry.getStatus().name());
                
                return ResponseEntity.ok(response);
            } else {
                Map<String, Object> response = new HashMap<>();
                response.put("message", "No sync needed - inquiry not PURCHASED");
                response.put("inquiryStatus", inquiry.getStatus().name());
                response.put("propertyStatus", property.getStatus().name());
                return ResponseEntity.ok(response);
            }
            ...
            System.err.println("Error syncing property status: " + e.getMessage());
            e.printStackTrace();
