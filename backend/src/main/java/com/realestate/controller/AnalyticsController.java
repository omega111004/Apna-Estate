...
public class AnalyticsController {
    ...
    @GetMapping("/public/homepage-stats")
    public ResponseEntity<Map<String, Object>> getHomepageStats() {
        Map<String, Object> stats = new HashMap<>();
        
        // Total properties count (only approved are visible publicly)
        long totalProperties = propertyRepository.findAllApproved().size();
        stats.put("totalProperties", totalProperties);
        
        // Cities covered count (only approved listings)
        List<String> cities = propertyRepository.findDistinctApprovedCities();
        stats.put("citiesCovered", cities.size());
        
        // Customer satisfaction (static for now, can be calculated from reviews later)
        stats.put("customerSatisfaction", 98);
