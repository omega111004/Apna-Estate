...
public class BookingAnalyticsController {
    ...
    public ResponseEntity<?> getAdminStats() {
        ...
        // Booking stats
        stats.put("totalBookings", rentBookingRepo.count());
        stats.put("activeBookings", rentBookingRepo.countByStatus(RentBooking.BookingStatus.ACTIVE));
        stats.put("pendingBookings", rentBookingRepo.countByStatus(RentBooking.BookingStatus.PENDING_APPROVAL));
        
        // Payment stats
        stats.put("totalRevenue", monthlyPaymentRepo.sumRevenueBetween(
            LocalDateTime.now().minusYears(100), LocalDateTime.now()));
        stats.put("pendingPayments", monthlyPaymentRepo.countByStatus(MonthlyPayment.PaymentStatus.PENDING));
        stats.put("overduePayments", monthlyPaymentRepo.countByStatus(MonthlyPayment.PaymentStatus.OVERDUE));
    ...
    public ResponseEntity<?> getOwnerStats() {
        ...
        // Booking stats
        List<RentBooking> ownerBookings = rentBookingRepo.findByOwner_Id(user.getId());
        stats.put("totalBookings", ownerBookings.size());
        stats.put("activeBookings", ownerBookings.stream()
            .filter(b -> b.getStatus() == RentBooking.BookingStatus.ACTIVE).count());
        stats.put("pendingBookings", ownerBookings.stream()
            .filter(b -> b.getStatus() == RentBooking.BookingStatus.PENDING_APPROVAL).count());
            
        // Payment stats
        stats.put("totalRevenue", monthlyPaymentRepo.sumRevenueBetweenForOwner(
            user.getId(), LocalDateTime.now().minusYears(100), LocalDateTime.now()));
        stats.put("pendingPayments", monthlyPaymentRepo.countPendingByOwner(user.getId()));
        stats.put("overduePayments", monthlyPaymentRepo.countOverdueByOwner(user.getId()));
        
        // Property stats
        stats.put("totalProperties", propertyRepo.countByOwner_Id(user.getId()));
        stats.put("rentedProperties", propertyRepo.countByOwner_IdAndStatus(
            user.getId(), Property.PropertyStatus.RENTED));
    ...
    public ResponseEntity<?> getTenantAnalytics() {
        ...
        tenantStats.put("activeTenants", rentBookingRepo.countDistinctActiveTenants());
        LocalDate firstOfMonth = LocalDate.now().withDayOfMonth(1);
        tenantStats.put("newTenantsThisMonth", userRepo.countCreatedSince(firstOfMonth.atStartOfDay()));
        
        tenantStats.put("topRatedTenants", Collections.emptyList());
