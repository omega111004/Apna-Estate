...
import { Users, Shield, Briefcase, User, Loader2, Trash2, Pencil, CheckCircle2, XCircle } from 'lucide-react';
...
const AdminUsersPage = () => {
    ...
    const toggleStatus = async (u, enabled) => {
        try {
            setBusyId(u.id);
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (<tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500"><Loader2 className="w-5 h-5 inline animate-spin mr-2"/> Loading users...</td></tr>) : users.length === 0 ? (<tr><td colSpan={4} className="px-6 py-6 text-center text-gray-500">No users found</td></tr>) : users.map((u) => (<tr key={u.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {u.enabled ? (<span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Active</span>) : (<span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-200 text-gray-700">Inactive</span>)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                      <button onClick={() => openEdit(u)} className="text-primary-600 hover:text-primary-900 inline-flex items-center">
                        <Pencil className="w-4 h-4 mr-1"/> Edit
                      </button>
                      {u.enabled ? (<button onClick={() => toggleStatus(u, false)} disabled={busyId === u.id} className="text-red-600 hover:text-red-900 inline-flex items-center disabled:opacity-50">
                          <XCircle className="w-4 h-4 mr-1"/> Deactivate
                        </button>) : (<button onClick={() => toggleStatus(u, true)} disabled={busyId === u.id} className="text-green-600 hover:text-green-900 inline-flex items-center disabled:opacity-50">
        ...
        {editing && (<div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} className="input-field md:col-span-2">
