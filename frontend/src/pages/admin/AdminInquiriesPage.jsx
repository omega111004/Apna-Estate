...
import { Loader2, MessageSquare, Eye, Calendar, Home, AlertCircle, Users, Filter, Search, Download, BarChart3, IndianRupee } from 'lucide-react';
...
const AdminInquiriesPage = () => {
    ...
    useEffect(() => {
        ...
        if (searchQuery) {
            ...
            filtered = filtered.filter(inquiry => inquiry.property.title.toLowerCase().includes(query) ||
    ...
    if (user?.role !== 'ADMIN') {
        ...
        <div className="text-center max-w-md">
          ...
          <button onClick={() => navigate('/dashboard')} className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Go to Dashboard
          </button>
        </div>
      </div>);
    ...
    return (<div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center flex-wrap gap-2">
                ...
                <span className="truncate">Admin - All Property Inquiries</span>
              </h1>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <button onClick={handleExportData} disabled={filteredInquiries.length === 0} className="inline-flex items-center justify-center px-4 py-3 sm:py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 min-h-[44px] w-full sm:w-auto">
                <Download className="w-4 h-4 mr-2"/>
                Export CSV
              </button>
              
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full flex-shrink-0 ${wsConnected ? 'bg-green-500' : 'bg-red-500'}`}/>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0 lg:space-x-4">
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input type="text" placeholder="Search by property, client, owner, or location..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
              </div>
            </div>
        ...
        {/* Empty state */}
        {!loading && !error && filteredInquiries.length === 0 && (<div className="text-center py-16">
            ...
            {searchQuery || statusFilter !== 'ALL' || dateFilter !== 'ALL' ? (<button onClick={() => {
                    setSearchQuery('');
                    setStatusFilter('ALL');
                    setDateFilter('ALL');
                }} className="inline-flex items-center justify-center px-4 py-3 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 min-h-[44px] w-full sm:w-auto">
                Clear Filters
              </button>) : null}
          </div>)}

        {/* Inquiries list */}
        {!loading && !error && filteredInquiries.length > 0 && (<div className="space-y-6">
            {filteredInquiries.map((inquiry) => (<div key={inquiry.id} className="bg-white rounded-xl shadow-sm border hover:shadow-md transition-shadow">
                <div className="p-6">
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                    {/* Main Info */}
                    <div className="flex-1 w-full">
                      <div className="flex items-start space-x-4">
                        {inquiry.property.imageUrl && (<img src={inquiry.property.imageUrl} alt={inquiry.property.title} className="w-16 h-16 object-cover rounded-lg flex-shrink-0"/>)}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                            ...
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border flex-shrink-0 ${getStatusColor(inquiry.status)}`}>
                              {inquiry.status}
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div className="bg-blue-50 rounded-lg p-3">
                              ...
                              {inquiry.client.email && (<div className="text-xs text-blue-600">{inquiry.client.email}</div>)}
                            </div>
                            ...
                            <div className="bg-green-50 rounded-lg p-3">
                              ...
                              {inquiry.owner.email && (<div className="text-xs text-green-600">{inquiry.owner.email}</div>)}
                            </div>
                          </div>
                          
                          {/* Price Info */}
                          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-6 space-y-1 sm:space-y-0 text-sm mb-3">
                            <span className="flex items-center text-gray-600">
                              <Home className="w-4 h-4 mr-1 flex-shrink-0"/>
                              <span className="truncate">Listed: {formatCurrency(inquiry.property.price)}</span>
                            </span>
                            ...
                            {inquiry.offeredPrice && (<span className="flex items-center text-blue-600 font-medium">
                                ...
                                <span className="truncate">Offered: {formatCurrency(inquiry.offeredPrice)}</span>
                              </span>)}
                            
                            {inquiry.agreedPrice && (<span className="flex items-center text-green-600 font-semibold">
                                <Calendar className="w-4 h-4 mr-1 flex-shrink-0"/>
                                <span className="truncate">Agreed: {formatCurrency(inquiry.agreedPrice)}</span>
                              </span>)}
                          </div>
                          
                          {/* Location and dates */}
                          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-xs text-gray-500">
                            {inquiry.property.city && (<span className="truncate">{inquiry.property.city}, {inquiry.property.state}</span>)}
                    ...
                    {/* Actions */}
                    <div className="flex flex-col items-start sm:items-end space-y-2 w-full sm:w-auto sm:ml-4">
                      <button onClick={() => handleViewInquiry(inquiry.id)} className="inline-flex items-center justify-center px-4 py-3 sm:py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors w-full sm:w-auto min-h-[44px]">
                        <Eye className="w-4 h-4 mr-2 sm:mr-2"/>
                        View Details
                      </button>
                      
                      {inquiry.status === 'PURCHASED' && (<span className="inline-flex items-center px-3 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                          Sale Completed
                        </span>)}
                    </div>
                  </div>
