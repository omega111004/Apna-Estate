...
const WalletPage = () => {
    ...
    const addMoney = async () => {
        ...
        try {
            ...
            if (!createOrder.ok)
                throw new Error(await createOrder.text());
            const orderData = await createOrder.json();
            const amountInPaise = orderData.amount; // from server
            const options = {
    ...
    return (<div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!isAuthenticated ? (<div className="py-20 text-center text-gray-500">
            Please log in to view your wallet.
          </div>) : loading ? (<div className="py-20 text-center text-gray-500">
            ...
            <div className="bg-white rounded-xl shadow-card p-6 mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input className="input-field" placeholder="Amount (₹)" value={addAmount} onChange={(e) => setAddAmount(e.target.value)} type="number" min="1"/>
                <input className="input-field md:col-span-2" placeholder="Description (optional)" value={addDescription} onChange={(e) => setAddDescription(e.target.value)}/>
              </div>
              ...
              <div className="mt-4">
                <button onClick={addMoney} disabled={busy} className="btn-primary disabled:opacity-50">
                ...
                <div className="mt-4">
                  ...
                  {showDiag && (<div className="mt-2 p-3 bg-gray-50 border rounded text-xs text-gray-700 space-y-2">
                      ...
                      {diagMsg && (<pre className="whitespace-pre-wrap break-words bg-white border rounded p-2 max-h-48 overflow-auto">{diagMsg}</pre>)}
                    </div>)}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-card p-6">
              ...
              {transactions.length === 0 ? (<div className="text-center text-gray-500 py-8">No transactions yet.</div>) : (<div className="space-y-3">
                  {transactions.map(txn => (<div key={txn.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center">
                        {txn.type === 'CREDIT' ? (<ArrowUpCircle className="w-5 h-5 text-green-600 mr-3"/>) : (<ArrowDownCircle className="w-5 h-5 text-red-600 mr-3"/>)}
                        <div>
