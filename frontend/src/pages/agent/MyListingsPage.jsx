...
import { Loader2, Home, BedSingle, Pencil, UserPlus } from 'lucide-react';
...
const MyListingsPage = () => {
    ...
    return (<div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading && (<div className="flex items-center justify-center py-20 text-gray-500">
        ...
        {!loading && items.length === 0 && (<div className="text-center text-gray-600 py-20">
            You have no listings yet.
          </div>)}

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((p) => (<div key={p.id} className="bg-white rounded-xl shadow-card overflow-hidden flex flex-col">
              {p.imageUrl ? (<img src={p.imageUrl} alt={p.title} className="h-44 w-full object-cover"/>) : (<div className="h-44 w-full bg-gray-100 flex items-center justify-center text-gray-400">
                  No Image
                </div>)}
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    {(user?.role === 'ADMIN' || user?.role === 'AGENT') && (<a href={`/edit-property/${p.id}`} className="inline-flex items-center text-gray-700 hover:text-gray-900" title="Edit">
                        <Pencil className="w-4 h-4 mr-1"/> Edit
                      </a>)}
                    {(user?.role === 'ADMIN' || user?.role === 'AGENT') && !p.owner && (<button onClick={async () => {
                    try {
                ...
                {(user?.role === 'ADMIN' || user?.role === 'AGENT') && p.isPgListing && (<div className="mt-3">
