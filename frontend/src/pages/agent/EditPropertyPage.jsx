    ...
    return (<div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          {(ownerId || ownerInfo || user) && (<div className="mt-4 p-4 bg-gray-50 border rounded-xl">
              ...
              <div className="text-sm text-gray-700 flex flex-col sm:flex-row sm:items-center sm:gap-4">
                <div>
                  {(ownerInfo?.firstName || ownerInfo?.lastName) ? (<span>{ownerInfo?.firstName} {ownerInfo?.lastName}</span>) : (<span>{(user?.firstName || '') + ' ' + (user?.lastName || '')}</span>)}
                </div>
                ...
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <span>{ownerInfo?.phone || user?.phoneNumber || 'Not set'}</span>
                </div>
          ...
          {(user && (user.role === 'ADMIN' || user.role === 'AGENT') && ownerId !== user.id) && (<div className="mt-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div>
                ...
                <select name="priceType" value={property.priceType} onChange={handleInputChange} className={`w-full input-field ${errors.priceType ? 'border-red-500' : ''}`}>
                  {Object.values(PriceType).map((pt) => (<option key={pt} value={pt}>{pt}</option>))}
                </select>
