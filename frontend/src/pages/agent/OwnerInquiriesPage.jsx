...
import { Loader2, MessageSquare, Calendar, Home, AlertCircle, Users, Bell, IndianRupee } from 'lucide-react';
...
const OwnerInquiriesPage = () => {
    ...
    const getPriorityLevel = (inquiry) => {
        ...
        if (inquiry.status === 'AGREED')
            return 'high';
        if (inquiry.status === 'NEGOTIATING' && hoursDiff < 24)
            return 'medium';
        if (inquiry.status === 'ACTIVE' && hoursDiff > 48)
            return 'low';
        return 'normal';
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center flex-wrap gap-2">
                ...
                <span className="truncate">Property Inquiries</span>
                {newInquiryCount > 0 && (<span className="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium bg-red-100 text-red-800 flex-shrink-0">
        ...
        <div className="mb-6">
          <div className="border-b border-gray-200 overflow-x-auto">
            <nav className="-mb-px flex space-x-4 sm:space-x-8 min-w-max">
              {['ALL', 'ACTIVE', 'NEGOTIATING', 'AGREED', 'PURCHASED', 'CANCELLED'].map((status) => (<button key={status} onClick={() => setStatusFilter(status)} className={`py-2 px-1 border-b-2 font-medium text-xs sm:text-sm whitespace-nowrap ${statusFilter === status
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
                  {status}
                  {status !== 'ALL' && statusCounts[status] && (<span className="ml-1 sm:ml-2 inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                      {statusCounts[status]}
                    </span>)}
                  {status === 'ALL' && (<span className="ml-1 sm:ml-2 inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                      {statusCounts.total}
                    </span>)}
                </button>))}
            </nav>
        ...
        {!loading && !error && filteredInquiries.length > 0 && (<div className="space-y-6">
            {filteredInquiries.map((inquiry) => {
                ...
                return (<div key={inquiry.id} className={`bg-white rounded-xl shadow-sm border-l-4 hover:shadow-md transition-shadow ${priority === 'high' ? 'border-l-green-500' :
                  ...
                  <div className="p-6">
                    <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="bg-gray-50 rounded-lg p-3 mb-3">
                              ...
                              <div className="flex items-center space-x-4 text-sm text-gray-600">
                                ...
                                {inquiry.client.email && (<a href={`mailto:${inquiry.client.email}`} className="text-blue-600 hover:underline">
                                    {inquiry.client.email}
                                  </a>)}
                                {inquiry.client.phone && (<a href={`tel:${inquiry.client.phone}`} className="text-blue-600 hover:underline">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-sm text-gray-600 mb-3">
                              <span className="flex items-center">
                                ...
                                <span className="truncate">Listed Price: ₹{inquiry.property.price.toLocaleString()}</span>
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-sm mb-3">
                              {inquiry.offeredPrice && (<span className="flex items-center text-blue-600 font-medium">
                                  ...
                                  <span className="truncate">Client Offer: ₹{inquiry.offeredPrice.toLocaleString()}</span>
                                </span>)}
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-xs text-gray-500">
                              ...
                              <span className="truncate">Last activity: {getTimeSinceUpdate(inquiry.updatedAt)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-start sm:items-end space-y-3 w-full sm:w-auto">
                        <div className="flex items-center space-x-2 flex-wrap">
                          {priority === 'high' && (<span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                              High Priority
                            </span>)}
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                          <button onClick={() => handleViewInquiry(inquiry.id)} className="inline-flex items-center justify-center px-4 py-3 sm:px-4 sm:py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors w-full sm:w-auto min-h-[44px]">
                            <MessageSquare className="w-4 h-4 mr-2 sm:mr-1"/>
                            Open Chat
                          </button>
                    ...
                    {/* Action indicators */}
                    {inquiry.status === 'ACTIVE' && (<div className="mt-4 pt-4 border-t border-gray-100">
                    ...
                    {inquiry.status === 'NEGOTIATING' && (<div className="mt-4 pt-4 border-t border-yellow-100 bg-yellow-50 -mx-6 -mb-6 px-6 py-3 rounded-b-xl">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-sm">
