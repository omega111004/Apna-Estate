...
import { BedDouble, BedSingle, Plus, Trash2, Loader2, Building } from 'lucide-react';
const ManagePgPage = () => {
    const { id } = useParams();
    ...
    const [roomForm, setRoomForm] = useState({ roomNumber: '', description: '', roomType: 'PRIVATE', roomCategory: 'FAMILY' });
    ...
    const loadRooms = async () => {
        try {
            ...
            const shared = data.filter((r) => r.roomType === 'SHARED');
            const bedMap = {};
            for (const r of shared) {
                const br = await fetch(`${apiBase}/pg/beds/room/${r.id}`, { headers });
                if (br.ok)
                    bedMap[r.id] = await br.json();
            }
            setBeds(bedMap);
        }
        catch (e) {
    ...
    useEffect(() => { if (propertyId)
        loadRooms(); /* eslint-disable-next-line */ }, [propertyId]);
    ...
    const deleteRoom = async (roomId) => {
        if (!confirm('Delete this room? All its beds will be removed.'))
            return;
        try {
    ...
    const addBed = async (roomId) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error('Add bed failed');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-xl shadow-card p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select className="input-field" value={roomForm.roomType} onChange={e => setRoomForm(f => ({ ...f, roomType: e.target.value }))}>
              <option value="PRIVATE">Private</option>
              <option value="SHARED">Shared</option>
            </select>
            <select className="input-field" value={roomForm.roomCategory} onChange={e => setRoomForm(f => ({ ...f, roomCategory: e.target.value }))}>
            ...
            {roomForm.roomType === 'PRIVATE' ? (<input className="input-field" placeholder="Private room price" value={roomForm.privateRoomPrice || ''} onChange={e => setRoomForm(f => ({ ...f, privateRoomPrice: e.target.value }))}/>) : (<>
        ...
        <div className="bg-white rounded-xl shadow-card">
          ...
          <div className="divide-y">
            {!loading && rooms.length === 0 && (<div className="px-4 py-8 text-center text-gray-500">No rooms yet</div>)}
            {rooms.map(r => (<div key={r.id} className="p-4">
                ...
                {/* Beds for SHARED */}
                {r.roomType === 'SHARED' && (<div className="mt-4 bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                      {(beds[r.id] || []).map(b => (<div key={b.id} className="flex items-center justify-between bg-white rounded border px-2 py-1 text-sm">
