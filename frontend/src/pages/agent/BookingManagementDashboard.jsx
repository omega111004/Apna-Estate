...
import { Bell, Users, Home, Calendar, AlertTriangle, CheckCircle, Clock, XCircle, Star, BarChart3, RefreshCw, Download, Search, IndianRupee } from 'lucide-react';
...
const BookingManagementDashboard = () => {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            const [pendingApprovalsRes, notificationsRes, unreadCountRes] = await Promise.all([
    ...
    const getNotificationIcon = (type) => {
        switch (type) {
            ...
            case 'PAYMENT_DUE': return <IndianRupee className="h-4 w-4"/>;
    ...
    if (error) {
        ...
        <div className="text-center">
          ...
          <button onClick={loadDashboardData} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
            Retry
          </button>
        </div>
      </div>);
    ...
    return (<div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <button onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 text-gray-600 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-lg">
              ...
              {/* Refresh Button */}
              <button onClick={loadDashboardData} disabled={refreshing} className="flex items-center px-4 py-2 text-gray-600 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {quickStats.map((stat, index) => (<motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">{stat.label}</p>
                  <p className="text-2xl font-bold text-gray-900 mt-2">{stat.value}</p>
                  <p className={`text-sm mt-2 ${stat.trend === 'up' ? 'text-green-600' :
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <button onClick={() => window.location.href = '/booking-approvals'} className="flex flex-col items-center p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                  <Clock className="h-6 w-6 text-blue-600 mb-2"/>
                  <span className="text-sm font-medium text-blue-900">Approvals</span>
                  {stats.pendingApprovals > 0 && (<span className="text-xs bg-blue-600 text-white px-2 py-1 rounded-full mt-1">
                      {stats.pendingApprovals}
                    </span>)}
                </button>
                
                <button onClick={() => window.location.href = '/bookings'} className="flex flex-col items-center p-4 bg-green-50 rounded-lg hover:bg-green-100 transition-colors">
                  <Home className="h-6 w-6 text-green-600 mb-2"/>
                  <span className="text-sm font-medium text-green-900">My Bookings</span>
                </button>
                
                <button onClick={() => window.location.href = '/properties'} className="flex flex-col items-center p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                {getFilteredNotifications().slice(0, 5).map((notification) => (<div key={notification.id} className={`p-4 rounded-lg border-l-4 ${getNotificationColor(notification.priority, notification.type)} ${!notification.isRead ? 'shadow-sm' : 'opacity-70'}`}>
                    <div className="flex items-start justify-between">
                      ...
                      {!notification.isRead && (<button onClick={() => markNotificationAsRead(notification.id)} className="text-blue-600 hover:text-blue-800 text-sm">
                          Mark read
                        </button>)}
                    </div>
                  </div>))}
            ...
            <div className="bg-white rounded-xl shadow-sm">
              <div className="p-6 border-b border-gray-200">
                ...
                <div className="flex space-x-2">
                  {['all', 'unread', 'high'].map((filter) => (<button key={filter} onClick={() => setNotificationFilter(filter)} className={`px-3 py-1 rounded-full text-sm capitalize ${notificationFilter === filter
                ? 'bg-blue-100 text-blue-800'
                : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                      {filter}
                      {filter === 'unread' && unreadCount > 0 && (<span className="ml-1 bg-blue-600 text-white text-xs rounded-full px-1">
                          {unreadCount}
                        </span>)}
                    </button>))}
                </div>
              ...
              <div className="max-h-96 overflow-y-auto">
                {getFilteredNotifications().map((notification) => (<div key={notification.id} className={`p-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${!notification.isRead ? 'bg-blue-50' : ''}`} onClick={() => !notification.isRead && markNotificationAsRead(notification.id)}>
                    <div className="flex items-start space-x-3">
                      <div className="text-gray-600 mt-1">
                        {getNotificationIcon(notification.type)}
                      </div>
                      ...
                      {notification.priority === 'HIGH' && (<span className="inline-block w-2 h-2 bg-red-500 rounded-full"></span>)}
                    </div>
                  </div>))}
                
                {getFilteredNotifications().length === 0 && (<div className="p-8 text-center">
                    <Bell className="h-8 w-8 text-gray-400 mx-auto mb-3"/>
